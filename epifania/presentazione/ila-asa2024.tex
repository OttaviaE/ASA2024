...
The algorithm iterates the following steps until the termination criterion is reached: 
	\begin{enumerate}
		...
		$\text{argmin}_{i \in \{1, \ldots, N\}\setminus Q} |\theta_{target} -b_i|$.
...
\begin{frame}
	...
	$L = N-1$: Maximum length of the STFs that can developed from the item bank composed of $N$ items
	
 For each of the possible lengths $l = \{1, \ldots L\}$ of the STF, a $\binom{N}{l}$ number of STFs is developed. 
