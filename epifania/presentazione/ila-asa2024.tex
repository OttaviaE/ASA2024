...
\author{ \vspace{-5mm} Ottavia M. Epifiania\textsuperscript{1}, Pasquale Anselmi\textsuperscript{3}, Egidio Robusto\textsuperscript{3}}
...
\textbf{\textcolor{unipd}{Issue}}
 ...
 A priori defintion of latent trait levels of interest on which the STF should be focusing the most
...
\begin{frame}
	57\% $\rightarrow$ $||Q_{\text{BFP}}|| - ||Q_{\text{ILA}}|| = 0$ % $\rightarrow$ 72\% same item selection
	\begin{figure}
		\centering
...
\begin{frame}
	...
	\begin{exampleblock}{\color{myGreen}\textbf{Pros}}
		
			\begin{itemize}
			\item ILA selects items that are able to recreate the desired characteristics of a test (usually)
			\item It is computationally ``Light'' 
		\end{itemize}
		
	\end{exampleblock}
	
	\begin{alertblock}{\color{unipd}Cons}
		
		\begin{itemize}
			\item ILA grounds its selection on a single $\theta_{target}$ $\rightarrow$ it might select items minimizing the distance on that target but that are useless for the test 
			\item ILA only forwardly searches an item $\rightarrow$ once it is in, it can't get out
			\item ILA does not account for the discrimination parameters of the items
		\end{itemize}
	\end{alertblock}
